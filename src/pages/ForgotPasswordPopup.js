...
import { FiX, FiMail, FiLock, FiEyeOff, FiEye } from 'react-icons/fi'
...
const ForgotPasswordPopup = ({ onClose }) => {
  ...
  const resetPassword = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/auth/forgot/reset`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim(), otp: otpValue, newPassword: pwd })
      })
      const data = await res.json().catch(() => ({}))
      if (res.ok) {
  ...
  return (
    <div className="fp-overlay" onMouseDown={stop} onClick={stop}>
        ...
        {step === 'otp' && (
          <>
            ...
            <div className="fp-row">
              <button className="fp-btn fp-ghost" onClick={() => setStep('email')}>
                Back
              </button>
        ...
        {step === 'reset' && (
          <>
            ...
            <div className="fp-input">
              ...
              <input type={showPwd ? 'text' : 'password'} placeholder="New Password" value={pwd} onChange={(e) => setPwd(e.target.value)} />
            ...
            <div className="fp-input">
              ...
              <input type={showCpwd ? 'text' : 'password'} placeholder="Confirm Password" value={cpwd} onChange={(e) => setCpwd(e.target.value)} />
            ...
            <div className="fp-row">
              <button className="fp-btn fp-ghost" onClick={() => setStep('otp')}>
                Back
              </button>
